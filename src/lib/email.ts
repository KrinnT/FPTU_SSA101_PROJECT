...
export async function sendVerificationEmail(to: string, code: string) {
    ...
    try {
        const info = await transporter.sendMail({
            from: process.env.SMTP_FROM || '"Psych Support" <no-reply@example.com>',
            to,
            subject: "Your Verification Code - Psych Support",
            text: `Your verification code is: ${code}`,
            html: `
                <div style="font-family: Arial, sans-serif; padding: 20px; background-color: #f4f4f4; border-radius: 8px;">
                    <h2 style="color: #333;">Welcome to Psych Support!</h2>
